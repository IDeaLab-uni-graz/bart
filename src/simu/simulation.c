...
void debug_sim(struct sim_data *data) {
  ...
  debug_printf(DP_INFO, "\tR2_2:%f \tR2_3:%f\t R2_4:%f\t R2_5:%f\n\n",
               data->voxel.r2[1], data->voxel.r2[2], data->voxel.r2[3],
               data->voxel.r2[4]);
  debug_printf(DP_INFO, "\tR1_2:%f \tR1_3:%f\t R1_4:%f\t R1_5:%f\n\n",
               data->voxel.r1[1], data->voxel.r1[2], data->voxel.r1[3],
               data->voxel.r1[4]);
  debug_printf(DP_INFO, "\tM0_2:%f \tM0_3:%f \tM0_4:%f \tM0_5:%f\n",
               data->voxel.m0[1], data->voxel.m0[2], data->voxel.m0[3],
               data->voxel.m0[4]);
  debug_printf(DP_INFO, "\tOm_2:%f \tOm_3:%f \tOm_4:%f \tOm_5:%f\n",
               data->voxel.Om[1], data->voxel.Om[2], data->voxel.Om[3],
               data->voxel.Om[4]);
...
void pulse_init(struct simdata_pulse *pulse, float rf_start, float rf_end,
                float angle /*[deg]*/, float phase, float bwtp, float alpha) {
...
static void compute_fields(struct sim_data *data, float gb_eff[3], float t) {
  ...
  if (data->seq.pulse_applied) {
    ...
    // dM/dt = M x (e_x*B_1*cos(phase)-e_y*B_1*sin(phase) +e_z* B_0)) - ...
    gb_eff[0] = crealf(w1);
    gb_eff[1] = -cimagf(w1);
  }
}
...
static void bloch_simu_stm_fun(struct sim_data *data, float r2spoil, int N,
                               float *out, float t, const float *in) {
  float gb_eff[3];
  ...
  if (N == 4) { // M
    ...
    bloch_mcc_matrix_ode_sa(data->voxel.P, matrix_time, data->voxel.r1, r2,
    ...
    bloch_mcc_matrix_ode_sa2(data->voxel.P, matrix_time, data->voxel.r1, r2,
                             data->voxel.k, data->voxel.m0, data->voxel.Om,
                             gb_eff, w1);
...
void mat_exp_simu(struct sim_data *data, float r2spoil, int N, float st,
                  float end, float out[N][N]) {
...
static void create_sim_matrix(struct sim_data *data, int N, float matrix[N][N],
                              float st, float end, float r2spoil) {
  if (data->seq.pulse_applied)
...
static void adc_corr(int P, int pools, float out[P][pools][3],
                     const float in[P][3 * pools], float angle) {
  for (int i = 0; i < P; i++) {
...
static void collect_signal(struct sim_data *data, int P, int pools,
                           float (*m)[pools][3], float (*sa_r1)[pools][3],
                           float (*sa_r2)[pools][3], float (*sa_b1)[1][3],
                           float (*sa_m0)[pools][3], float (*sa_k)[pools][3],
                           float (*sa_Om)[pools][3], float xp[P][pools * 3]) {
  float tmp[P][pools][3];
  ...
  for (int p = 0; p < pools; p++) {
    for (int i = 0; i < 3; i++) {
      ...
      if ((1 < pools) &&
          (NULL != sa_m0)) // For 1 == pools -> sa_m0 == m, see sum_up_signal()
...
static void sum_up_signal(
    float m0, int R, int S, int A, float D, int pools,
    float (*m)[R * A][S][pools][3], float (*sa_r1)[R * A][S][pools][3],
    float (*sa_r2)[R * A][S][pools][3], float (*sa_b1)[R * A][S][1][3],
    float (*sa_m0)[R * A][S][pools][3], float (*sa_k)[R * A][S][pools][3],
    float (*sa_om)[R * A][S][pools][3], float (*m_state)[R][pools][3],
    float (*sa_r1_state)[R][pools][3], float (*sa_r2_state)[R][pools][3],
    float (*sa_m0_state)[R][pools][3], float (*sa_b1_state)[R][1][3],
    float (*sa_k_state)[R][pools][3], float (*sa_om_state)[R][pools][3]) {
...
static void hard_pulse(struct sim_data *data, int N, int P, float xp[P][N]) {
  for (int i = 0; i < P; i++)
    bloch_excitation2(xp[i], xp[i],
                      DEG2RAD(CAST_UP(&data->pulse.sinc)->flipangle),
                      data->pulse.phase);
}
  ...
  if (0. == data->pulse.rf_end) {
    ...
    for (; t_im <= data->pulse.rf_end; t_im += sample_time) {
      ...
      for (int i = 0; i < P; i++) {
        ...
        bloch_relaxation(xp[i], sample_time, xp3, data->voxel.r1[0],
                         data->voxel.r2[0], data->grad.gb);
      }
    }
  }
}

void rf_pulse(struct sim_data *data, float h, float tol, int N, int P,
              float xp[P][N], float stm_matrix[P * N][P * N]) {
  ...
  // Define effective z Gradient = Slice-selection gradient + off-resonance
  // [rad/s]
  ...
  case SIM_ODE: {
    ...
    NESTED(void, call_pdy2, (float *out, float t, const float *in)) {
      ...
      if (MODEL_BMC == data->seq.model) {
        ...
        bloch_mcc_pdy(data->voxel.P, (float(*)[N])out, in, data->voxel.r1,
                      data->voxel.r2, data->voxel.k, data->voxel.m0,
                      data->voxel.Om, gb_eff);

      } else {

        bloch_pdy((float(*)[3])out, in, data->voxel.r1[0], data->voxel.r2[0],
                  gb_eff);
      }
    };
    ...
    NESTED(void, call_pdp2, (float *out, float t, const float *in)) {
      ...
      if (MODEL_BMC == data->seq.model) {
        ...
        bloch_mcc_b1_pdp(data->voxel.P, (float(*)[N])out, in, data->voxel.r1,
                         data->voxel.r2, data->voxel.k, data->voxel.m0, gb_eff,
                         w1);

      } else {

        bloch_b1_pdp((float(*)[3])out, in, data->voxel.r1[0], data->voxel.r2[0],
                     gb_eff, w1);
      }
    };

    // Choose P-1 because ODE interface treats signal separate and P only
    // describes the number of parameters
    ode_direct_sa(h, tol, N, P - 1, xp, data->pulse.rf_start,
                  data->pulse.rf_end, call_fun, call_pdy2, call_pdp2);

  } break;

  case SIM_STM:

    create_sim_matrix(data, P * N, stm_matrix, data->pulse.rf_start,
                      data->pulse.rf_end, 0.);
    break;
  }
...
static void hard_relaxation(struct sim_data *data, int N, int P, float xp[P][N],
                            float st, float end, float r2spoil) {
  assert(0. <= (end - st));
  ...
  for (int i = 0; i < P; i++) {
    ...
    bloch_relaxation(xp[i], end - st, xp2, data->voxel.r1[0],
                     data->voxel.r2[0] + r2spoil, data->grad.gb);
  }
}

void relaxation2(struct sim_data *data, float h, float tol, int N, int P,
                 float xp[P][N], float st, float end,
                 float stm_matrix[P * N][P * N], float r2spoil) {
  ...
  case SIM_ODE: {
    ...
    debug_printf(DP_INFO, "Relaxation");
    if (r2spoil > 0) {
      debug_printf(DP_INFO, " with spoiling %f\n", r2spoil);
    } else {
    ...
    NESTED(void, call_pdy2, (float *out, float t, const float *in)) {
      ...
      if (MODEL_BMC == data->seq.model) {
        ...
        bloch_mcc_pdy(data->voxel.P, (float(*)[N])out, in, data->voxel.r1, r2,
                      data->voxel.k, data->voxel.m0, data->voxel.Om, gb_eff);

      } else {

        bloch_pdy((float(*)[3])out, in, data->voxel.r1[0],
                  data->voxel.r2[0] + r2spoil, gb_eff);
      }
    };
    ...
    NESTED(void, call_pdp2, (float *out, float t, const float *in)) {
      ...
      if (MODEL_BMC == data->seq.model) {
        ...
        bloch_mcc_b1_pdp(data->voxel.P, (float(*)[N])out, in, data->voxel.r1,
                         r2, data->voxel.k, data->voxel.m0, gb_eff, w1);

      } else {

        bloch_b1_pdp((float(*)[3])out, in, data->voxel.r1[0],
                     data->voxel.r2[0] + r2spoil, gb_eff, w1);
      }
    };

    // Choose P-1 because ODE interface treats signal separate and P only
    // describes the number of parameters
    ode_direct_sa(h, tol, N, P - 1, xp, st, end, call_fun, call_pdy2,
                  call_pdp2);
...
static void prepare_sim(struct sim_data *data, int N, int P,
                        float (*mte)[P * N + 1][P * N + 1],
                        float (*mtr)[P * N + 1][P * N + 1]) {
  switch (data->seq.type) {
  ...
  case SIM_STM:
    ...
    if ((0 != data->grad.mom_sl) && (data->seq.te != data->pulse.rf_end)) {
      ...
      relaxation2(data, 0, 0, M, 1, NULL, data->pulse.rf_end, data->seq.te,
                  mrel, 0.);
...
static void run_sim(struct sim_data *data, int pools, float (*mxy)[pools][3],
                    float (*sa_r1)[pools][3], float (*sa_r2)[pools][3],
                    float (*sa_b1)[1][3], float (*sa_m0)[pools][3],
                    float (*sa_k)[pools][3], float (*sa_Om)[pools][3], float h,
                    float tol, int N, int P, float xp[P][N],
                    float xstm[P * N + 1], float mte[P * N + 1][P * N + 1],
                    float mtr[P * N + 1][P * N + 1]) {
  float r2spoil = 0.;
  ...
  case SIM_ODE:
    ...
    collect_signal(data, P, pools, mxy, sa_r1, sa_r2, sa_b1, sa_m0, sa_k, sa_Om,
                   xp);
...
void inversion(const struct sim_data *data, float h, float tol, int N, int P,
               float xp[P][N], float st, float end) {
...
static void alpha_half_preparation(const struct sim_data *data, int pools,
  ...
  CAST_UP(&prep_data.pulse.sinc)->flipangle =
      CAST_UP(&data->pulse.sinc)->flipangle / 2.;
  ...
  if (0. < data->seq.prep_pulse_length) {
    ...
    run_sim(&prep_data, pools, NULL, NULL, NULL, NULL, NULL, NULL, NULL, h, tol,
            N, P, xp, NULL, NULL, NULL);

  } else { // Perfect preparation

    for (int p = 0; p < P; p++)
      bloch_excitation2(xp[p], xp[p],
                        DEG2RAD(CAST_UP(&prep_data.pulse.sinc)->flipangle),
                        prep_data.pulse.phase);
  }
}
...
static void cest_seq(struct sim_data *data, float h, float tol, int N, int P,
                     float xp[P][N], float offset) {
...
void bloch_simulation2(const struct sim_data *_data, int R, int pools,
                       float (*m_state)[R][pools][3],
                       float (*sa_r1_state)[R][pools][3],
                       float (*sa_r2_state)[R][pools][3],
                       float (*sa_m0_state)[R][pools][3],
                       float (*sa_b1_state)[R][1][3],
                       float (*sa_k_state)[R][pools][3],
                       float (*sa_om_state)[R][pools][3]) {
  // FIXME: split config + variable part

  struct sim_data data =
      *_data; // Lose information of complex pointer variables
  ...
  int P = (1 == pools)
              ? 4
              : 5 * pools; // Number of parameters with estimated derivative (M,
                           // DR1, DR2, DB1, DM0, Dk, DOm)

  assert(0 < P);

  int M = N * P + 1; // STM based on single vector and additional +1 for
  ...
  //              FIXME: Slice Profile in relative units of theoretical slice
  //              thickness?
  // debug_printf(DP_INFO, "Theoretical Slice Thickness: %f\n", 2 * M_PI /
  // ((data.pulse.rf_end - data.pulse.rf_start) / (2. + (data.pulse.nl - 1.) +
  // (data.pulse.nr - 1.)) * data.grad.sl_gradient_strength * GAMMA_H1));
  ...
  for (int s = 0; s < S; s++) {
    ...
    if (1 != S) {
      ...
      // w_{gz} [rad/s] = gamma_H1 [rad/(sT)] * Gz [T/m] * z [m], FIXME: Change
      // name of variable mom_sl
      data.grad.mom_sl = (_data->grad.sl_gradient_strength *
                          _data->seq.slice_thickness * GAMMA_H1) /
                         (S - 1) * (s - (int)(S / 2.));
    ...
    if ((SEQ_CEST == data.seq.seq_type) && data.cest.ref_scan) {
      ...
      cest_seq(&data, h, tol, N, P, xp,
               data.cest.ref_scan_ppm * 2 * M_PI * data.cest.b0 *
                   data.cest.gamma);
    ...
    for (int r = 0; r < data.seq.rep_num; r++) {
      ...
      if (SEQ_CEST == data.seq.seq_type) {
        ...
        collect_signal(&data, P, pools, mxy, sa_r1, sa_r2, sa_b1, sa_m0, sa_k,
                       sa_Om, xp);
        reset_xp(N, data.voxel.P, xp, data.voxel.m0);
  ...
  //      - Relative to default slice thickness to keep strength of simulation
  //      higher
  float D = (float)S / (data.seq.slice_thickness / default_slice_thickness);

  sum_up_signal(data.voxel.m0[0], data.seq.rep_num / A, data.seq.spin_num, A, D,
                pools, Fmxy, Fsa_r1, Fsa_r2, Fsa_b1, Fsa_m0, Fsa_k, Fsa_Om,
                m_state, sa_r1_state, sa_r2_state, sa_m0_state, sa_b1_state,
                sa_k_state, sa_om_state);
...
void bloch_simulation(const struct sim_data *data, int R, float (*m)[R][3],
                      float (*sa_r1)[R][3], float (*sa_r2)[R][3],
                      float (*sa_m0)[R][3], float (*sa_b1)[R][3]) {
  bloch_simulation2(data, R, 1, (void *)m, (void *)sa_r1, (void *)sa_r2,
                    (void *)sa_m0, (void *)sa_b1, NULL, NULL);
}
